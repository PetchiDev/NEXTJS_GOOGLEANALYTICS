...
const BannerCarousel: React.FC<BannerCarouselProps> = ({ banners, loading }) => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && activeBanner.id) {
            const bannerId = activeBanner.code || `content_news_hero_${activeBanner.id}`;
            ...
            trackBannerInteraction(bannerId, 'view', 'hero_banner');
  ...
  const handleBannerClick = () => {
    if (activeBanner?.href && activeBanner.id) {
      ...
      trackBannerInteraction(bannerId, 'click', 'hero_banner');
